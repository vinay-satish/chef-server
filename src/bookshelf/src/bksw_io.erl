...
bucket_create(Bucket) ->
    %?debugFmt("~nbksw_io:bucket_create(~p)", [Bucket]),
    case bucket_exists(Bucket) of
     ...
     %               ?debugFmt("~nbksw_io: created bucket '~p' at '~p'", [Bucket, BucketPath]),
                    true;
                {error, Reason} ->
                    ?LOG_ERROR("Error creating bucket ~p on path ~p: ~p~n", [Bucket, BucketPath, Reason]),
      %              ?debugFmt("~nbksw_io: ERROR creating bucket '~p' on path ~p: ~p~n", [Bucket, BucketPath, Reason]),
                    false
            end;
        true ->
       %     ?debugFmt("~nbksw_io:bucket_create - already exists"),
            true
    end.
...
delete_bucket_dir(Bucket) ->
    %?debugFmt("~nbksw_io:bucket_delete(~p)", [Bucket]),
    case os:cmd("rm -rf " ++ bksw_io_names:bucket_path(binary_to_list(Bucket))) of
        [] ->
            ?LOG_DEBUG("deleted bucket ~p", [Bucket]),
     %       ?debugFmt("~nbksw_io:bucket_delete - deleted bucket ~p", [Bucket]),
            true;
        Why ->
            ?LOG_ERROR("bucket delete failed for bucket ~p: ~p", [Bucket, Why]),
     %       ?debugFmt("~nbksw_io:bucket_delete - failed for bucket ~p: ~p", [Bucket, Why]),
            false
    end.
...
%                    ?debugFmt("~nbksw_io:open_for_write successful"),
...
%                    ?debugFmt("~nbksw_io: header write failed ~p ~p at ~p: ~p", [Bucket, Entry, FileName, Error]),
                    file:close(Fd),
                    Error
            end;
        Error ->
            ?LOG_ERROR("open_for_write failed ~p ~p at ~p: ~p",
                       [Bucket, Entry, FileName, Error]),
%            ?debugFmt("bksw_io:open_for_write failed ~p ~p at ~p: ~p", [Bucket, Entry, FileName, Error]),
            Error
    end.

-spec open_for_read(binary(), binary()) -> {ok, #entryref{}} | {error, term()}.
open_for_read(Bucket, Entry) ->
%    ?debugFmt("~nbksw_io:open_for_read ..."),
%    ?debugFmt("~nBucket = ~p", [Bucket]),
%    ?debugFmt("~nEntry  = ~p", [Entry]),
...
%            ?debugFmt("~nfile:open successful"),
            case file:read(Fd, 2) of
                ...
                {ok, ?MAGIC_NUMBER} ->
                    ...
                    {ok, ?TOTAL_HEADER_SIZE_BYTES} = file:position(Fd, {bof, ?TOTAL_HEADER_SIZE_BYTES}),
%                    ?debugFmt("~nbksw_io:open_for_read successful, file ok"),
...
%                    ?debugFmt(":nbksw_io:open_for_read ERROR corrupt file ~p ~p ~p at ~p", [Bucket, Entry, FileName, ReadError]),
                    file:close(Fd),
                    {error, corrupt_file}
            end;
        Error ->
...
%?debugFmt("~nbksw_io:write... "),
    case file:write(Fd, Data) of
        ok ->
%?debugFmt("~nbksw_io:write succeeded"),
            {ok, ERef#entryref{ctx=erlang:md5_update(Ctx, Data)}};
        Error ->
%?debugFmt("~nbksw_io:write failed ~p ~p: ~p", [Bucket, Entry, Error]),
...
%?debugFmt("~nbksw_io:finish_write..."),
    case file:sync(Fd) of
        ok ->
%?debugFmt("~nbksw_io:finish_write file:sync succeeded"),
            ...
            case filelib:ensure_dir(FinalPath) of
                ok ->
                    case file:rename(Path, FinalPath) of
                        ok ->
%?debugFmt("~nbksw_io:finish_write completed ~p ~p", [Bucket, Entry]),
                            ?LOG_DEBUG("write completed ~p ~p", [Bucket, Entry]),
                            {ok, Digest};
                        Error ->
%?debugFmt("~nbksw_io:finish_write rename failed  ~p ~p (~p > ~p): ~p", [Bucket, Entry, Path, FinalPath, Error]),
                            ?LOG_ERROR("rename failed ~p ~p (~p > ~p): ~p",
                                       [Bucket, Entry, Path, FinalPath, Error]),
                            Error
                    end;
                DirError ->
%?debugFmt("~nbksw_io:finish_write mkdir failed  ~p ~p (~p): ~p", [Bucket, Entry, FinalPath, DirError]),
                    ?LOG_ERROR("mkdir failed ~p ~p (~p): ~p",
                               [Bucket, Entry, FinalPath, DirError]),
                    DirError
            end;
        Error ->
%?debugFmt("~nbksw_io:finish_write sync failed  ~p ~p: ~p", [Bucket, Entry, Error]),
