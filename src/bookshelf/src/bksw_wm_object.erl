...
resource_exists(Rq0, Ctx) ->
    ...
    case wrq:method(Rq0) of
        'PUT' ->
            %?debugFmt("~nbksw_wm_object: ok, resource exists for PUT"),
            {true, Rq0, Ctx};
        _ ->
            ...
            case bksw_io:open_for_read(Bucket, Path) of
                {error, enoent} ->
                    %?debugFmt("~nbksw_wm_object: error enoent"),
                    {false, Rq0, Ctx};
                {ok, Ref} ->
                    %?debugFmt("~nbksw_wm_object: ok, resource exists"),
...
delete_resource(Rq0, Ctx) ->
    %?debugFmt("~nbksw_wm_object:delete_resource ..."),
    {ok, Bucket, Path} = bksw_util:get_object_and_bucket(Rq0),
    Res = bksw_io:entry_delete(Bucket, Path),
    case Res of
        true ->
            %?debugFmt("~nbksw_wm_object:delete_resource successful"),
            {true, Rq0, Ctx};
        Other ->
            %?debugFmt("~nbksw_wm_object:delete_resource ERROR - ~p", [Other]),
            {Other, Rq0, Ctx}
    end.
...
upload(Rq0, Ctx) ->
    %?debugFmt("~nbksw_wm_object:upload attempting bksw_util:get_object_and_bucket..."),
X = bksw_util:get_object_and_bucket(Rq0),
%?debugFmt("~nget_object_and_bucket result = ~p", [X]),
    {ok, Bucket, Path} = X,
    case bksw_io:open_for_write(Bucket, Path) of
        {ok, Ref} ->
%?debugFmt("~nbksw_wm_object:upload - attempting write_streamed_body"),
%?debugFmt("~nRq0: ~p", [Rq0]),
%?debugFmt("~n?BLOCK_SIZE:: ~p", [?BLOCK_SIZE]),
            % BLOWS UP HERE AT WEBMACHINE
            Resp = write_streamed_body(wrq:stream_req_body(Rq0, ?BLOCK_SIZE), Ref, Rq0, Ctx),
            % NEVER GETS HERE!
            %?debugFmt("~nbksw_wm_object:upload finished"),
            Resp;
        Error ->
            % NEVER GETS HERE!
            error_logger:error_msg("Error opening ~p/~p for writing: ~p~n", [Bucket, Path, Error]),
            %?debugFmt("~nbksw_wm_object:upload ERROR = ~p Bucket = ~p Path = ~p", [Error, Bucket, Path]),
            {false, Rq0, Ctx}
    end.
...
%?debugFmt("~nbksw_wm_object:write_streamed_body (done)..."),
%?debugFmt("~nbksw_wm_object: attempting bksw_io:write"),
    {ok, Ref1} = bksw_io:write(Ref, Data),
%?debugFmt("~nbksw_wm_object: bksw_io:write successful, attemping bksw_io:finish_write"),
    {ok, Digest} = bksw_io:finish_write(Ref1),
%?debugFmt("~nbksw_wm_object: bksw_io:finish_write successful"),
...
%?debugFmt("~nbksw_wm_object:write_streamed_body (chunk)..."),
%?debugFmt("~nbksw_wm_object: attempting bksw_io:write"),
    {ok, Ref1} = bksw_io:write(Ref, Data),
%?debugFmt("~nbksw_wm_object: bksw_io:write successful"),
